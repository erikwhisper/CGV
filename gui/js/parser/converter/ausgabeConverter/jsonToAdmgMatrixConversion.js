/**
 * @description Macht aus JSON Objekt ADMG MAtrix
 * @param {JSON} jsonData
  ...
  jsonData.links.forEach((link) => {
    ...
    if (
      ...
      matrix[sourceIndex][targetIndex] =
        matrix[sourceIndex][targetIndex] === 2 ? 4 : 2;
      matrix[targetIndex][sourceIndex] =
        matrix[targetIndex][sourceIndex] === 2 ? 5 : 3;
    } else if (
      link.arrowhead === "tail" &&
      link.arrowtail === "normal" &&
      !link.isDashed
      ...
      //B -> A
      matrix[sourceIndex][targetIndex] =
        matrix[sourceIndex][targetIndex] === 2 ? 5 : 3;
      matrix[targetIndex][sourceIndex] =
        matrix[targetIndex][sourceIndex] === 2 ? 4 : 2;
    }
  });

  //diesen prettyfie call für die matrix in utils pacjen und einf reusen?.
